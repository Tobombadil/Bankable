...
The intake-approval route (`POST .../approve-intake`) and its dedicated helpers moved to
`services/api/admin_intake.py` (docs/42-backend-review-2026-09-26.md §7 lane L4); that module
imports `_find_task`, `_require_reason` and `serialize_task` back from here.

Decisions (fuller versions with reasons in `services/api/admin_people.md`; the intake-approval
decisions moved to `services/api/admin_intake.py`'s own docstring with it):

1. `Task.subject_id`'s schema (`AnyPublicIdValue`) accepts only `prop_ | opp_ | org_ | mat_ | evt_ |
...
2. A user's own API keys (`created_by_user_id`), not every key on their account, are revoked on
   disable/delete/redaction — deleting one member of a multi-user account must not cut the other
   members' credentials.
3. `POST /admin/v1/customers`'s `primary_contact_email` is, per its own schema description,
...
4. Every CRM/billing adapter call in this module runs inside the request's one DB session
...
def admin_create_customer(
    ...
    domain = normalise_domain(organization.website) if organization is not None else None
