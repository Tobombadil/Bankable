...
class Connector(BaseConnector):
    ...
    dq_required_fields: ClassVar[tuple[str, ...]] = ("name_canonical", "sponsor_name", "capacity_mw", "technology_raw")
    key_source_columns: ClassVar[tuple[str, ...]] = ("Project ID", "Project Name", "Project Status", "Plant Type",
                                                     "Cumulative Total Capacity (MW)", "MW Effective From")

    def fetch(self) -> RawSnapshot:
        t0 = time.monotonic()
        pkg = self.http.get(PACKAGE_URL, honour_robots=False).json()
        resources = [r for r in pkg.get("result", {}).get("resources", []) if str(r.get("format", "")).upper() == "CSV"]
        if not resources:
        ...
        return RawSnapshot(content=r.content, content_type=r.headers.get("Content-Type", ""), url=res["url"],
                           retrieved_at=dt.datetime.now(dt.UTC), http_status=r.status_code, ext="csv",
                           headers=dict(r.headers), elapsed_s=round(time.monotonic() - t0, 2), requests_made=2,
                           meta={"resource_id": res.get("id"), "last_modified": res.get("last_modified"),
                                 "licence": pkg.get("result", {}).get("license_title"), "dataset_page": DATASET_PAGE})
