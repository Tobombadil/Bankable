...
from pipeline.connectors.opportunity import classify_technologies, deadline_passed, iso2, technologies_str, to_utc
...
SECTOR_TECH = {"LU": "solar_pv", "LH": "hydro", "LW": "wind", "LB": "biomass", "LI": "geothermal",
               "LT": "transmission", "LN": "thermal"}
FIELDS = ["id", "notice_type", "noticedate", "notice_status", "submission_deadline_date", "submission_deadline_time",
          "project_ctry_code", "project_ctry_name", "project_id", "project_name", "regionname", "sector",
          "agency_name", "bid_reference_no", "bid_currency_code", "bid_estimate_amount", "bid_description",
          "procurement_group", "procurement_group_desc", "procurement_method_code", "procurement_method_name",
          "unspsc_classification", "submission_date", "noticetitle", "api_modified_date", "market_approach_name",
          "notice_lang_code", "notice_version_no", "contact_organization"]
OPEN_TYPES = {"Invitation for Bids", "Request for Bids", "Request for Proposals", "Request for Expression of Interest",
              "Invitation for Prequalification", "Specific Procurement Notice"}
...
class Connector(BaseConnector):
    ...
    key_source_columns: ClassVar[tuple[str, ...]] = ("id", "notice_type", "noticedate", "noticetitle", "project_ctry_code")

    def params(self, offset: int) -> dict[str, Any]:
        return {"format": "json", "rows": PAGE, "os": offset, "sector.sector_code": "^".join(SECTOR_CODES),
                "srt": "noticedate", "order": "desc", "fl": ",".join(FIELDS)}
    ...
    def fetch(self) -> RawSnapshot:
        ...
        return RawSnapshot(content=payload, content_type="application/json", url=url.split("&os=")[0], retrieved_at=now,
                           http_status=200, ext="json", elapsed_s=round(time.monotonic() - t0, 2),
                           requests_made=len(pages), meta={"total": pages[0].get("total"), "pages": len(pages),
        ...
        for page in doc.get("pages", []):
            ...
            for n in _notices(page):
                ...
                if not nid or nid in seen:
                    continue
                seen.add(nid)
                rows.append({k: v for k, v in n.items() if not (k.startswith("contact_") and k != "contact_organization")})
        return rows
    ...
    def normalize(self, rows: list[dict[str, Any]], raw: RawSnapshot) -> pd.DataFrame:
        ...
        for r in rows:
            ...
            ctx = {"status_raw": ntype, "deadline_passed": deadline_passed(due, now),
            ...
            recs.append({
                ...
                "due_at": due,
                "status": state, "status_raw": ntype, "status_rule": rule,
                "identifiers": json.dumps({"wb_notice_id": r.get("id"), "wb_project_id": r.get("project_id"),
                                           "bid_reference_no": r.get("bid_reference_no"), "sector_codes": sectors}),
            })
