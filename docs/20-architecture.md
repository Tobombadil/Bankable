...
|---|---|---|---|
| `public` | anonymous, or free account | **Every record and every change event as it commits** (amended 2026-09-19, closed 2026-09-21); derived fields only; restricted sources aggregated or linked out | Row-level predicate in the store layer applied to every query; page cache keyed by day |
| `pro` | account with entitlement `pro` (session or API key) | The same rows, no sooner; plus alerts, exports, saved searches — shape, not freshness | Entitlement check on session/key |
...
- ~~`lag` is configuration, default **7 days** **[A-8]** (`docs/01` §3.4 gives a 7–30 day range). It can be set
  ...
  freshness premium to protect (`docs/01` §7).~~ **Amended 2026-09-19 and closed 2026-09-21 (owner).** There is
  no lag on any tier, for any shape, and no configuration for one: the manifest field, the `source.lag_days` /
  `source.lag_overrides` columns and the admin `PATCH` fields were all removed with the delay (migration
  `0019`), so reintroducing one is a code change rather than a setting. `docs/21` §5.4 carries the full
  statement; the 7-vs-14 disagreement between **[A-8]** and `docs/10` A-7 is closed by the decision rather than
  settled.
- Licence gating is orthogonal to tier and stricter: a `licence` row states whether derived fields, raw fields
  ...
  (`CLAUDE.md`).
- Public pages carry a visible "every record and every change event is published as soon as it is ingested"
  line and a machine-readable `data_as_of` field (`docs/04` D-3/D-28, amended with the decision).
- ~~The delayed feed is served from a materialised view refreshed hourly; the live feed reads the base tables.~~
  There is one feed and one code path, since there is nothing to hold back; caching is a performance choice now,
  not a tier mechanism.
...
| A-8 | ~~Public delay 7 days by default, per-source-class override~~ **Retired 2026-09-19, and the remainder 2026-09-21: no delay of any kind, and no knob** | Owner decisions (paywall by shape; then the measurement that the surviving ISO delay was recoverable from two public reads) | `services/ingest/lag.py`, migrations `0016` and `0019` |
