...
| `us.doe.oced.portfolio`, `us.doe.netl.carbon_storage_portfolio` | public-domain | raw-ok | 17 U.S.C. §105; OCED portfolio is client-rendered (browser task), NETL carbon-storage pages answered 403 "You are not authorized to access this page" (unpublished, not an edge block) on 2026-09-25; added 2026-09-25 | high |
| `us.phmsa.hazardous_liquid_annual` | public-domain | raw-ok | 17 U.S.C. §105; same reasoning as `us.phmsa.pipeline_operator_reports` and §2.16 for the archive route; NOT yet fetched (origin 403 honoured; one archive attempt on 2026-09-25 ended in a connection reset, not retried); added 2026-09-25 | high |
| `us.sec.edgar_fts` | permissive | derived-only | SEC "Accessing EDGAR Data", read 2026-09-25: "Anyone can access and download this information for free"; "Current max request rate: 10 requests/second"; "Please declare your user agent in request headers"; filings are registrant-authored, so headline fact, filer, date and link only — no filing bodies; added 2026-09-25 | mod-high |
| `no.sodir.factpages_co2_storage` | open-attribution | derived-only + credit to the Norwegian Offshore Directorate until the NLOD text is read | FactPages "Use of content", read 2026-09-25: "The content on the FactPages may be used in accordance with Norwegian Licence for Open Government Data (NLOD). In this context we emphasize that there may be limitations with respect to the use of information subject to third party rights, such as reports, core images and logs etc." NLOD 2.0 itself (data.norge.no/nlod/en) not yet retrieved — browser task; added 2026-09-25 | mod |
| `us.nd.dmr.oilgas`, `us.wy.ogcc.data`, `us.ok.occ.well_data`, `us.nm.ocd.data`, `us.ks.kgs.wells`, `us.il.dnr.oilgas`, `us.sd.puc.co2_pipeline_dockets`, `us.nd.psc.case_search`, `us.ia.iuc.efs`, `us.il.icc.edocket`, `us.mn.puc.edockets` | **unknown** | do not ingest; gated until each state's terms are read | State oil-and-gas commissions, a university survey (KGS) and state PUC docket systems — none a federal work. Probed 2026-09-25 with per-source evidence in `data/sources.yaml`: Oklahoma's only notice is a vendor DMCA procedure; Iowa's EFS (`efs.iowa.gov`) and Wyoming's legacy data site (`pipeline.wyo.gov`) carry robots `Disallow: /` and were not fetched; Minnesota eDockets answered a bare 403 | n/a |
| `gb.nsta.carbon_storage_licences`, `global.iea.ccus_projects` | **unknown** | do not ingest; gated until terms read | NSTA data pages reached (2026-09-25) but no licence clause located in the served HTML (ArcGIS Hub is client-rendered); IEA product page and `/terms` answered a Cloudflare challenge, reported as a block; added 2026-09-25 | n/a |
