...
**L3 outcome (2026-09-26).** `web/app.py` 1,421 → 733 lines (49 → 24 functions; longest 86 → 54); new `web/assets_pages.py` (319 lines: `assets_list`, `asset_detail`, `asset_by_public_id` and the index's six constants and four helpers grep showed only they reach) and `web/sitemaps.py` (205 lines: the three routes and the closed builder cluster, plus `SITEMAP_STATIC_PATHS`, `ROBOTS_DISALLOW` and the lifecycle-states CSV only it used); `web/page.py` 555 → 826 because `_asset_extras` is also called by `search`, which stays, so it and the presenter cluster under it (`_fuel_fields` 86 — pre-existing, moved as-is — `_normalise_owners`, `_feedstock`, `_host_landfill`, `_diameter_text`, `_quantity`, `_year`, `_padd` and five constants) went to the shared module, as §4.1 had them. One route the brief named was correctly **not** moved: `asset_detail_proxy` at `/api/assets/{public_id}` reaches nothing the page cluster reaches and, included where the brief said, would register ahead of the literal `/api/assets/geo` and swallow it (`web/test_asset_pages.py` pins that literal); it stays at its original position. Verified by AST: all 74 original `web/app.py` definitions and all 44 `web/page.py` definitions node-identical in their new homes (decorators aside); the only new name is `router`. Repo counts unchanged (pure move: functions 1,914, ≥ 80 lines 49, nesting ≥ 5 8 before L4 landed); one test import retargeted (`web/test_seo.py`). Since the review: `web/app.py` 2,554 → 733.

**L5 outcome (2026-09-26).** `services/api/app.py` 1,896 → 1,072 lines (51 → 27 functions); new `services/api/records.py` (892 lines, 23 functions, longest 56) holds the ten proposal/opportunity routes and the filter stack. What stayed in `app.py`: app construction, `standard_headers`, the `include_router` block, organisations, events, feeds, health, meta, coverage, licences, sources, lifecycle-states. Nine names both sides need (`PROPOSAL_FILTERS`, `OPPORTUNITY_FILTERS`, the two sort allow-lists, the two `*_query_with_filters`, the two `*_licence_rows`, `_opportunity_technologies_filter`) live in `records.py` and are imported by `app.py` — the only cycle-safe direction. §2 item 4 closed: `list_proposal_events` and `list_opportunity_events` were AST-identical after substituting five proposal/opportunity tokens and are one `_list_subject_events(config, …)` with two thin routes over a five-field `_SubjectEventsConfig`; duplicate groups 3 → 2. The three generic request helpers (`LIST_COMMON`, `sort_spec`, `int_param`) moved once to `services/api/params.py` (fan-in 11 → 12) after a first pass had copied them into both files; the same `_int_param` body also existed, identical, in `assets.py` and `admin_sources.py` and now imports from `params.py` too. Verified by AST: of 69 original `app.py` definitions, 67 node-identical in their new home once the two helper renames are normalised; the two that differ are the merged event routes. The generated OpenAPI document is byte-identical before and after (127 operations, 7,749-line JSON diff empty); `api/openapi.yaml` untouched. `records.py` joins `app.py` in `pyproject.toml`'s per-file `B008` exemption instead of carrying twenty `noqa` markers. Two test targets moved with the code (`test_placement.py` import, `test_slippage.py` monkeypatch string). Max parameters among new functions 5.

**L6 outcome (2026-09-26).** Two contracts added to `infra/importlinter.ini`, four kept in total. `web-reads-through-the-api`: `web` must not import `services.ingest`, `services.resolve` or `services.db` directly (`allow_indirect_imports = True`, the shape the existing vendor-adapter contract uses, because `web → services.api → services.ingest.vintage` is the intended direction and must not count); the three production exceptions are exactly the composition roots §3 measured — `web.dev_up`, `web.data_loading`, `web.admin.records` — and thirteen `web/test_*` modules are listed explicitly because they wire a SQLite session or patch a loader in fixtures and import-linter's wildcard can only stand for a whole module segment (`web.*` would exempt production code too). `ingest-and-resolve-do-not-import-the-api`: zero edges when measured, now enforced. First draft failed for the right reasons and was reshaped, not loosened: it counted indirect chains and test modules; production code held the rule unchanged. Final run on the tree carrying L1–L5 (`018ec6c` + this change): `Contracts: 4 kept, 0 broken`, 38 ignored imports, all of them the listed exceptions.
...
- **Four `_int_param`s, one different** (lane L5): `services/api/app.py`, `assets.py` and `admin_sources.py` carried the same three-line helper (now one, `params.int_param`); `services/api/admin_records.py::_int_param` wraps `int()` in `try/except ValueError` and raises a 400 `validation_error`, where the shared one lets a non-integer query value surface as `int()`'s own error. Real behavioural divergence between admin and public routes; left as-is by a behaviour-preserving lane. Candidate for the same follow-up as the geo `bbox`/`zoom` item: decide which behaviour is right and make all four routes do it.
