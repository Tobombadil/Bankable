...
CarbonStorage (CCUS), Grid Status, Kadoa, datacenterHawk.
...
| CarbonStorage (carbonstorage.io) | CCUS-only tracker: Class VI, operational and planned storage, capture, EOR, carbon removal, CO₂ pipelines, e-fuels and stratigraphic wells, plus economic-analysis, capture-cost, PVT and unit-conversion tools (https://carbonstorage.io/). Two tiers, **both priced "Custom"**: Corporate, and Enterprise gated at "Designed for teams of 8 or more" with "API access for seamless data integration" and sponsorship slots (https://carbonstorage.io/pricing, read 2026-09-22). The feature list names "In-depth project pages with **FOIA results**, news coverage, permits, and downloadable files", "global stratigraphic wells" and a "Global dataset of point source emissions and emitting facilities". Their own footer strip (F, but marketing copy): "Founded 2024 / Countries 50+ / Global Team 5+ / Coverage Global". **They publish no data sources at all** — no methodology, /about, /data or /methodology page (all 404) — so none of the coverage claims is checkable from outside | Closest thing to a direct CCUS competitor found. Two things are worth copying and one is worth not believing. Copy: (a) **FOIA as a data source** — a paid feature we have never costed, and the only part of their stack that is not a public feed; (b) pairing storage projects with a point-source **emissions** dataset, which is the emitter↔storage join and is free in the US (EPA GHGRP, now `us.epa.ghgrp`). Do not believe, pending evidence: "50+ countries, global coverage" from a self-stated team of five founded in 2024. Their own homepage activity feed is the check available — its newest item is dated 2025-11-03, ~10 months stale on 2026-09-22 (three distinct dates across 16 entries, site "Version 4.10.5") | **Low-medium** — narrow vertical (CCUS only), no demand side, no public pricing, unverifiable coverage; but they own the CCUS niche's mindshare and could widen |
