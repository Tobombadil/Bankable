...
from pipeline.normalize import (classify_tech, harmonise_status, load_status_map,  # noqa: E402
                                norm_county, norm_name, norm_org, norm_state)
...
@pytest.mark.parametrize("src,ctx,expected,rule", [
    ("caiso", {"status_raw": "ACTIVE"}, "studied", "caiso.map"),
    ("caiso", {"status_raw": "ACTIVE", "ia_status": "Executed"}, "contracted", "caiso.active_ia_executed"),
    ("caiso", {"status_raw": "ACTIVE", "ia_status": "Filed Unexecuted"}, "permitted", "caiso.active_ia_filed"),
    ...
    ("ercot", {"status_raw": "Active", "approved_for_energization": "yes"}, "under_construction", "ercot.energized"),
    ...
    ("isone", {"status_raw": "Withdrawn", "project_status": "Under Study"}, "withdrawn", "isone.withdrawn_wins"),
    ("isone", {"status_raw": "Completed", "project_status": "In Service"}, "built", "isone.in_service"),
    ("isone", {"status_raw": "Completed", "project_status": "Under Construction"}, "under_construction", "isone.under_construction"),
    ("isone", {"status_raw": "Active", "ia_status": "Executed"}, "contracted", "isone.ia_executed"),
    ("isone", {"status_raw": "Active"}, "studied", "isone.map"),
    ("eia860m", {"status_raw": "(P) Planned for installation, but regulatory approvals not initiated"}, "announced", "eia860m.map"),
    ("eia860m", {"status_raw": "(L) Regulatory approvals pending. Not under construction"}, "filed", "eia860m.map"),
    ("eia860m", {"status_raw": "(T) Regulatory approvals received. Not under construction"}, "permitted", "eia860m.map"),
    ("eia860m", {"status_raw": "(U) Under construction, less than or equal to 50 percent complete"}, "under_construction", "eia860m.map"),
    ("eia860m", {"status_raw": "(V) Under construction, more than 50 percent complete"}, "under_construction", "eia860m.map"),
    ("eia860m", {"status_raw": "(TS) Construction complete, but not yet in commercial operation"}, "under_construction", "eia860m.map"),
])
def test_harmonise(src, ctx, expected, rule):
    assert harmonise_status(src, ctx, SM) == (expected, rule)


@pytest.mark.parametrize("orig,status,expected", [
...
def test_refine_rules_take_precedence_over_map_and_run_in_order():
    ...
    assert harmonise_status("isone", {"status_raw": "Withdrawn", "project_status": "In Service"}, SM)[0] == "withdrawn"


@pytest.mark.parametrize("raw,tech,kind", [
...
def test_normalisers():
    ...
    assert norm_county("Weld County") == "WELD" and norm_county("Kit Carson/Cheyenne Counties") == "KIT CARSON/CHEYENNE"
