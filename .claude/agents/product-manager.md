...
- Keep scope honest: the MVP is the proposal graph + change feed + delayed public tier + Pro alerts + syndication. The existing Bankable deal workflow is a later consideration, never a foundation (owner, 2026-09-12). Push back on anything that does not serve a named user job.
...
Working rules
- Users first: developers/IPPs, lenders and investors, utilities and co-ops, EPCs/OEMs, advisors, and the internal operations team. Name which user every feature serves.
